...
# ========================================================================
# CORRECTION: compute_simulation robuste (requires + inventaire final)
# ========================================================================
...
def compute_simulation(ir, ast):
    ...
    # Construire le graphe de dependances (unlocks + requires)
    ...
    for q in quests:
        ...
        if not qid:
            continue
        quest_map[qid] = q
        in_degree[qid]  # initialise a 0

    for q in quests:
        qid = q.get("id") if isinstance(q, dict) else None
        if not qid:
            continue
        # unlocks -> qid debloque unlocked
    ...
    # Ajouter les quetes restantes (inaccessibles ou cycles)
    ...
    # CORRECTION: retourner l'inventaire final, pas l'inventaire initial
    return {
        "order": order,
        "inventory": {
            "gold": inventory["gold"],
...
# ========================================================================
# CORRECTION: build_quest_graph robuste
# ========================================================================
...
def _extract_title(entity, fallback):
    """Extrait le titre d'une entite IR."""
...
# ========================================================================
# CORRECTION: build_passes_report simplifie et robuste
# ========================================================================
def build_passes_report(report):
    ...
    # Collecter toutes les entrees (errors + warnings)
    all_entries = []
...
def _get_error_code(entry):
    """Extrait le code d'erreur d'une entree."""
    if isinstance(entry, dict):
...
def _error_to_dict(entry, severity="error"):
    """Convertit une entree en dict uniforme."""
    if isinstance(entry, dict):
        result = dict(entry)
...
# ========================================================================
# CORRECTION: count_ast_nodes avec protection anti-cycles
# ========================================================================
...
def compile_code():
    ...
    try:
        ...
        print(f"[SERVER] Compilation terminee avec succes")
