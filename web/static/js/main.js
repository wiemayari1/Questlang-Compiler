(function() {
    ...
    function highlightCode(code) {
        ...
        lines.forEach(function(line, i) {
            ...
            text = text.replace(/(\/\/.*$)/g, function(m) { return addPH('<span class="token-comment">' + m + '</span>'); });
            text = text.replace(/("(?:[^"\\]|\\.)*")/g, function(m) { return addPH('<span class="token-string">' + m + '</span>'); });
            text = text.replace(/\b(\d+\.?\d*)\b/g, function(m) { return addPH('<span class="token-number">' + m + '</span>'); });
            keywords.forEach(function(kw) {
                var re = new RegExp('\\b(' + kw + ')\\b', 'g');
                text = text.replace(re, function(m) { return addPH('<span class="token-keyword">' + m + '</span>'); });
            });
            builtins.forEach(function(bi) {
                var re = new RegExp('\\b(' + bi + ')\\b', 'g');
                text = text.replace(re, function(m) { return addPH('<span class="token-builtin">' + m + '</span>'); });
            });
    ...
    function bindEvents() {
        ...
        if (btnHtml) btnHtml.addEventListener('click', exportHtmlReport);
    ...
    async function exportHtmlReport() {
        if (!currentCode || !currentCode.trim()) {
            log('Aucun code source a exporter.', 'warn');
            return;
        }
        log('Generation du rapport HTML en cours...', 'info');
        try {
            var r = await fetch(API + '/api/report/html', {
                method: 'POST',
            ...
            if (d.success && d.html) {
                ...
                a.href = url;
                a.download = 'rapport_questlang.html';
                a.click();
                URL.revokeObjectURL(url);
                log('Rapport HTML genere et telecharge.', 'ok');
            } else {
                log('Erreur generation HTML: ' + (d.error || 'Erreur inconnue'), 'err');
            }
        } catch (e) {
