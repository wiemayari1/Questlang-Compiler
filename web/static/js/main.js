(function() {
  ...
  function highlightCode(code) {
    ...
    lines.forEach(function(line, i) {
      ...
      html += '<div class="src-line"><span class="src-linenum">' + (i + 1) + '</span><span class="src-code">' + hl + '</span></div>';
    });
    return html;
  }
  ...
  function renderCompDetails(details) {
    ...
    if (content) {
      var html = '';
      html += '<div class="comp-detail-item"><span class="label">Total: </span><span class="value">' + (details.total_time || '-') + '</span></div>';
      html += '<div class="comp-detail-item"><span class="label">Tokens: </span><span class="value">' + (details.tokens_count || '-') + '</span></div>';
      html += '<div class="comp-detail-item"><span class="label">AST: </span><span class="value">' + (details.ast_nodes || '-') + ' noeuds</span></div>';
  ...
  function renderSim() {
    ...
    if (!simData) {
      if (timeline) timeline.innerHTML = '<div style="color:#555570;padding:20px;">Compilez pour simuler</div>';
    ...
    if (simIndex >= 0 && history[simIndex]) {
      ...
      if (Object.keys(items).length > 0) {
        ...
        Object.entries(items).forEach(function(kv) { body += '<span class="sim-reward">' + esc(kv[0]) + ' x' + kv[1] + '</span>'; });
        body += '</div>';
      }
  ...
  function renderTokens(tokens) {
    ...
    tbody.innerHTML = '';
    if (!tokens.length) {
      tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;color:#555570;padding:20px;">--</td></tr>';
      return;
    }
    tokens.forEach(function(t) {
      var tr = document.createElement('tr');
      tr.innerHTML = '<td>' + t.line + '</td><td>' + t.col + '</td><td class="tok-type">' + esc(t.type) + '</td><td>' + esc(String(t.value)) + '</td>';
  ...
  function resetAll() {
    ...
    var content = document.getElementById('comp-detail-content');
    if (content) content.innerHTML = '<p class="comp-placeholder">Chargez et compilez.</p>';
  ...
  function setCompiling(on) {
    ...
    if (!btn) return;
    if (on) { btn.innerHTML = '<span class="spin"></span>...'; btn.disabled = true; }
